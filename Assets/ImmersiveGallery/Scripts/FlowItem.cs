    ...
    public class FlowItem : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        public async void MoveToPlaceholderAsync(Transform targetAnchor, bool isCenter = false)
        {
            ...
            try
            {
                ...
                _moveTask.Add(transform.LerpPositionAsync(targetAnchor.position, _moveDuration, _cts.Token, _moveCurve));
                _moveTask.Add(transform.LerpRotationAsync(targetAnchor.rotation, _rotateDuration, _cts.Token, _rotateCurve));
                _moveTask.Add(transform.LerpScaleAsync(isCenter ? Vector3.one * _focusedSize : Vector3.one * _unfocusedSize, _scaleDuration, _cts.Token, _scaleCurve));
